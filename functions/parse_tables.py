...
def parse_tables(model, tables):
    ...
    for table in tables:
        ...
        with open(file=table, mode='r') as file:
            data = file.readlines()
            row_dict = {'model': model, 'table_name': table.split('/')[-1].replace('.tmdl', '')}
                ...
                for i, row in enumerate(partition_rows):
                    ...
                    for c, child_row in enumerate(data[row + 1:]):
                        ...
                        # Partition source
                            ...
                            while get_indent(data[current_source_expr_row]):
                                    source_expression.append(data[current_source_expr_row].strip())
                                    current_source_expr_row += 1
                            
                            # Format source expression
                            source_expression = ' '.join(source_expression)[''.join(source_expression).index('=') + 1:]
                            source_expression = source_expression.replace('```', '').strip()
                            # Assign the source expression
